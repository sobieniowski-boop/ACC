"""Full rollup Feb 1 - Mar 11 with v2 model data, 1-day batches."""
import sys, os, time
...
while d <= end:
    ...
    est_remaining = remaining * (total_elapsed / day_count) if day_count > 0 else 0
    print(f"SKU={sku} ({elapsed:.0f}s) [{day_count}/{(end-start).days+1}d, ~{est_remaining/60:.0f}min left]")
