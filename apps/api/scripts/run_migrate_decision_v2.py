"""Run Decision Intelligence migration on Azure SQL — v2, individual statements."""
import sys, os
...
def main():
    ...
    for i, stmt in enumerate(TABLES, 1):
        try:
            cur.execute(stmt)
            print(f"[{i}/4] Table OK")
        except Exception as e:
            print(f"[{i}/4] Table ERR: {e}")
    ...
    print("Migration OK!" if len(tables) == 4 else f"WARN: only {len(tables)} tables")
    cur.close()
    conn.close()
