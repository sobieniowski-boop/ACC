...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DHL seed/cost/aggregate/shadow from current staging.")
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting DHL seed pipeline")
    print(
        json.dumps(
            {
    ...
    for window_start, window_end in windows:
        seed_result = seed_dhl_shipments_from_staging(
