...
def main() -> int:
    parser = argparse.ArgumentParser(description="Run DHL cost/aggregate/shadow from current staging.")
    ...
    parser.add_argument("--replace-shadow", action="store_true", default=True)
    ...
    print(f"[{time.strftime('%Y-%m-%d %H:%M:%S')}] Starting DHL staging reconcile")
    print(
        json.dumps(
            {
    ...
    started = time.time()
    windows = _month_windows(created_from, created_to) if args.monthly_batches else [(created_from, created_to)]
    ...
    shadow_result = build_dhl_logistics_shadow(
        ...
        limit_orders=args.limit_orders,
        replace_all_existing=args.replace_shadow,
    )
